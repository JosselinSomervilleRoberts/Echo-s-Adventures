...
uniform float Ka = 1.0f; // Ambient coefficient
	...
	if(continuer==true) {
		...
		if(continuer==true) {
			...
			for(int k_light=0; k_light<N_lights; k_light++)
			{
				...
				// spotlight color
				color_shading += (Kd*diffuse*color_object + Ks * specular) * spotlight_color[k_light]*exp(-spotlight_falloff[k_light]*dist*dist);
			}

			// fog effect
			float depth1 = length(fragment.eye-fragment.position) / sqrt(R);
			float depth2 = sqrt(distance) / dist_zone;
			float w_depth = exp(-fog_falloff*depth1*depth1) * exp(-fog_falloff*depth2*depth2);
